...
fn decals_are_visible_and_centered() {
    ...
    let brick_entity = app.world_mut().spawn((BrickType::Standard, Decal::default())).id();
    // Query for decal and check position
    let decal = app.world().get::<Decal>(brick_entity).expect("Decal missing");
    // Replace with actual position/visibility logic
    assert!(decal.is_centered_on_top(), "Decal is not centered on top side");
