...
- Level-control keys (`R` = restart level, `L`/ `N` = next level, `P` = previous level, `K` = destroy all bricks) and debug tools (`Space` = wireframe) are gated to Cheat Mode: they only execute when Cheat Mode is active.
