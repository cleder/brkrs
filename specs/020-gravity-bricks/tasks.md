...
- [ ] T001 Define `GravityChanged` message and validation in `src/systems/gravity/mod.rs` with `#[derive(Message)]`
- [ ] T002 Define `GravityBrick` component marker in `src/components/gravity.rs` with index (21-25) and gravity vector
- [ ] T003 Define `GravityConfiguration` resource in `src/physics_config.rs` with `current` and `level_default` fields
- [ ] T004 Register `GravityChanged` message type in main app with `app.register_message::<GravityChanged>()`
- [ ] T005 Create `tests/gravity_bricks.rs` test module with test harness and utilities (setup, assertions, helpers)
...
- [ ] T006 Implement `gravity_configuration_loader_system` in `src/systems/gravity/mod.rs` that loads `default_gravity` from `LevelDefinition` and initializes `GravityConfiguration` resource
- [ ] T007 Register gravity systems in main app schedule: add `gravity_configuration_loader_system` to `Update`, `gravity_application_system` to `PhysicsUpdate`, `gravity_reset_on_life_loss_system` to `PostUpdate`
- [ ] T008 Extend `LevelDefinition` struct in `src/level_format/mod.rs` to include optional `default_gravity: Option<Vec3>` field; add RON deserialization support with `None` fallback to `Vec3::ZERO`
- [ ] T009 Verify level loading gracefully handles missing `default_gravity` field (backwards compatibility); test with existing levels that lack gravity config
...
- [ ] T010 [P] [US1] Write unit tests for gravity brick destruction detection in `tests/gravity_bricks.rs::test_gravity_brick_21_zero_gravity()` - assert brick 21 destruction sends `GravityChanged { gravity: (0.0, 0.0, 0.0) }`
- [ ] T011 [P] [US1] Write unit tests for gravity brick 22 (2G) in `tests/gravity_bricks.rs::test_gravity_brick_22_moon_gravity()` - assert sends message with `(0.0, 2.0, 0.0)`
- [ ] T012 [P] [US1] Write unit tests for gravity brick 23 (10G) in `tests/gravity_bricks.rs::test_gravity_brick_23_earth_gravity()` - assert sends message with `(0.0, 10.0, 0.0)`
- [ ] T013 [P] [US1] Write unit tests for gravity brick 24 (20G) in `tests/gravity_bricks.rs::test_gravity_brick_24_high_gravity()` - assert sends message with `(0.0, 20.0, 0.0)`
- [ ] T014 [P] [US1] Write unit tests for gravity application to ball physics in `tests/gravity_bricks.rs::test_gravity_applied_to_ball_velocity()` - assert ball's `Velocity` is affected by `GravityConfiguration::current` in next physics frame
- [ ] T015 [US1] Write integration test for gravity brick destruction flow in `tests/gravity_bricks.rs::test_destroy_gravity_brick_changes_gravity()` - setup level with gravity brick, spawn ball, trigger destruction, assert `GravityConfiguration::current` updated, verify ball physics response
- [ ] T016 [US1] Write test for message buffering/ordering in `tests/gravity_bricks.rs::test_multiple_gravity_bricks_sequential()` - destroy 3 bricks (21, 24, 22) in sequence, assert gravity transitions correctly (zero → high → light)

### Implementation for US1

- [ ] T017 [P] [US1] Implement `brick_destruction_gravity_handler` system in `src/systems/gravity/mod.rs` that detects when brick entities with `GravityBrick` component are destroyed (via change detection or despawn event)
- [ ] T018 [P] [US1] In `brick_destruction_gravity_handler`: Query for destroyed bricks with `GravityBrick` component, read gravity value from component, create `GravityChanged` message
- [ ] T019 [US1] In `brick_destruction_gravity_handler`: Write `GravityChanged` message via `MessageWriter<GravityChanged>` (depends on T017, T018)
- [ ] T020 [P] [US1] Implement `gravity_application_system` in `src/systems/gravity/mod.rs` that reads `MessageReader<GravityChanged>` and updates `GravityConfiguration::current` for each message received
- [ ] T021 [US1] Implement physics gravity application: modify ball's physics to use `GravityConfiguration::current` (integrate with Rapier 3D gravity/forces; see `src/physics_config.rs` for existing gravity handling pattern)
- [ ] T022 [US1] Add Bevy 0.17 compliance checks: verify no panicking queries (use `?` operator), verify queries filter with `With<Ball>`, `With<RigidBody>`, etc., verify no `.unwrap()` on physics entities
- [ ] T023 [US1] Update level RON files to include gravity bricks (21-25) in test level maps for manual verification
...
- [ ] T024 [P] [US2] Write unit test for ball loss detection in `tests/gravity_bricks.rs::test_gravity_reset_on_ball_loss()` - setup level with default gravity 10G, change to zero gravity, lose ball, assert `GravityConfiguration::current` reset to `(0.0, 10.0, 0.0)` before next ball spawn
- [ ] T025 [P] [US2] Write test for zero gravity fallback in `tests/gravity_bricks.rs::test_gravity_reset_to_zero_gravity_fallback()` - load level without `default_gravity` field, change gravity, lose ball, assert reset to `(0.0, 0.0, 0.0)`
- [ ] T026 [US2] Write integration test for full gravity lifecycle in `tests/gravity_bricks.rs::test_gravity_lifecycle_multiple_balls()` - destroy gravity bricks, lose balls, spawn new balls, verify gravity resets correctly each time

### Implementation for US2

- [ ] T027 [P] [US2] Implement `gravity_reset_on_life_loss_system` in `src/systems/gravity/mod.rs` that detects ball loss (listen to existing ball loss event/signal from ball_lives system)
- [ ] T028 [P] [US2] In `gravity_reset_on_life_loss_system`: Query `GravityConfiguration` resource, reset `current` to `level_default` value (depends on T027)
- [ ] T029 [US2] Ensure gravity reset happens before next ball spawns (schedule dependency: gravity reset in `PostUpdate` before ball respawn system)
- [ ] T030 [US2] Add logging/debug output for gravity reset events (optional: useful for debugging and gameplay feedback)
- [ ] T031 [US2] Verify Bevy 0.17 compliance: ensure life loss detection doesn't panic on queries, uses change detection correctly
...
- [ ] T032 [P] [US3] Write unit tests for gravity brick scores in `tests/gravity_bricks.rs::test_gravity_brick_21_score()` through `test_gravity_brick_25_score()` - assert each brick type awards correct points via existing score system
- [ ] T033 [US3] Write integration test for score update on destruction in `tests/gravity_bricks.rs::test_score_updated_on_gravity_brick_destruction()` - destroy gravity brick, verify score component updated immediately

### Implementation for US3

- [ ] T034 [P] [US3] Add score award logic to `brick_destruction_gravity_handler` (or existing brick destruction score system) - query destroyed brick with `GravityBrick` component, look up points from static map/table, send score message
- [ ] T035 [US3] Create gravity brick score lookup in `src/systems/gravity/mod.rs` or `src/systems/brick_destruction.rs`: index 21→125 pts, 22→75 pts, 23→125 pts, 24→150 pts, 25→250 pts
- [ ] T036 [US3] Ensure score updates integrate with existing score system (verify compatibility with `src/systems/[score-system]`)
...
- [ ] T037 [P] [US4] Write unit test for message queue buffering in `tests/gravity_bricks.rs::test_gravity_messages_buffered_in_order()` - send 3 `GravityChanged` messages, verify they process in order
- [ ] T038 [P] [US4] Write test for gravity state consistency in `tests/gravity_bricks.rs::test_last_gravity_wins_sequential_destruction()` - destroy bricks (21, 24, 22) in sequence, assert final gravity is from brick 22 (last destroyed)
- [ ] T039 [US4] Write integration test for rapid brick destruction in `tests/gravity_bricks.rs::test_rapid_multiple_brick_destruction()` - spawn ball, destroy 5 gravity bricks rapidly, verify physics remains stable and gravity applies correctly for each

### Implementation for US4

- [ ] T040 [P] [US4] Verify message system handles multiple messages per frame correctly (no additional code needed if using standard Bevy messages; verify in tests T037-T039)
- [ ] T041 [US4] Ensure `gravity_application_system` processes all buffered messages (MessageReader handles this automatically; verify with tests)
...
- [ ] T046 [P] Queer Gravity RNG independence test in `tests/gravity_bricks.rs::test_queer_gravity_no_correlation()` - generate 20+ random gravities, verify no obvious bias or correlation between consecutive values

### Implementation for Queer Gravity

- [ ] T047 [P] Implement RNG for Queer Gravity in `brick_destruction_gravity_handler` using `rand` crate - on brick 25 destruction, generate random Vec3 with specified ranges
- [ ] T048 [P] Add `use rand::Rng` and create thread-local or function-scoped RNG for random gravity generation
- [ ] T049 In `brick_destruction_gravity_handler`: Detect brick index 25, generate random gravity, send `GravityChanged` message with random value
